...
namespace ASDSWPFUI.ViewModel
{
    ...
    public sealed class NodeViewModel : AbstractModelBase
    {
        ...
        /// List of input connectors (connections points) attached to the node.
        /// </summary>
        private ImpObservableCollection<ConnectorViewModel> paramConnectors = null;
        ...
        /// List of input connectors (connections points) attached to the node.
        /// </summary>
        public ImpObservableCollection<ConnectorViewModel> ParamConnectors
        {
            get
            {
                if (paramConnectors == null)
                {
                    paramConnectors = new ImpObservableCollection<ConnectorViewModel>();
                    paramConnectors.ItemsAdded += new EventHandler<CollectionItemsChangedEventArgs>(paramConnectors_ItemsAdded);
                    paramConnectors.ItemsRemoved += new EventHandler<CollectionItemsChangedEventArgs>(paramConnectors_ItemsRemoved);
                }

                return paramConnectors;
            }
        }
        /// <summary>
        /// List of output connectors (connections points) attached to the node.
        ...
        public ICollection<ConnectionViewModel> AttachedConnections
        {
            get
            {
                ...
                foreach (var connector in this.ParamConnectors)
                {
                    attachedConnections.AddRange(connector.AttachedConnections);
                }
        ...
        /// Event raised when connectors are added to the node.
        /// </summary>
        private void paramConnectors_ItemsAdded(object sender, CollectionItemsChangedEventArgs e)
        {
            foreach (ConnectorViewModel connector in e.Items)
            {
                connector.ParentNode = this;
                connector.Type = ConnectorType.Param;
            }
        }
        ...
        /// Event raised when connectors are removed from the node.
        /// </summary>
        private void paramConnectors_ItemsRemoved(object sender, CollectionItemsChangedEventArgs e)
        {
            foreach (ConnectorViewModel connector in e.Items)
            {
                connector.ParentNode = null;
